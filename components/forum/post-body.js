...
function PostBody({post}) {
	...
	return (
		...
		>
			...
			{date_forum &&
			<section className={`appeals-forum bg-color-secondary ${events?.length === 0 ? "pt-4vh" : "py-4vh"}`}>
			...
			<section className="events-forum bg-color-brand shadow4 py-4vh">
				<Events
					events={events}
					className="events-container px-4vw my-4"
				/>
			</section>
			}
			...
			<section className="reviews-forum bg-color-primary py-4vh">
				<Reviews
					reviews={reviews}
					className="reviews-container px-4vw my-4"
				/>
			</section>
			}
