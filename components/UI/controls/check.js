...
const CheckControl = ({type, choices, title, name, required, inline}) => {
			...
			>
				{
					...
					<>
						<Form.Check.Input type={type} name={type === 'radio' ?  `${name}` : `${name}-${i}`} onClick={handleClick}/>
						<Form.Check.Label>{Object.values(value)}</Form.Check.Label>
						{type === 'textarea'
							? <Form.Control as={Object.keys(value)} rows={3} onClick={handleClick} onBlur={handleClick}/>
							: <Form.Control type={Object.keys(value)} onClick={handleClick} onBlur={handleClick}/>
						}
					</>
					) : null
				}
